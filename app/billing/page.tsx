...
import { Search, MoreVertical, Edit2, Trash2, ArrowRightCircle, FileText, CheckCircle2, ChevronLeft, ChevronRight, Download, X, Plus, Calculator, Wallet, CreditCard, Calendar, Clock, Percent, Printer, FileDown } from 'lucide-react';
...
export default function BillingPage() {
  ...
  const fetchBills = useCallback(async () => {
    try {
      const res = await fetch(`/api/billing?page=${billPage}&pageSize=${billPageSize}&search=${encodeURIComponent(billSearch)}`);
      const data = await res.json();
      setBills(data.data || []);
  ...
  const handleOpenBillModal = async () => {
    ...
    try {
      const res = await fetch('/api/billing/generate-preview', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleSaveBill = async () => {
    try {
      ...
      alert('Failed to save bill. Please check your connection or server status.');
    }
  };
  ...
  const handleViewInvoice = async (bill: Bill) => {
    try {
      // First check if 'partyId' is already a person object from server
      // or if we need to fetch party info.
      // In this system, partyId is the name.
      const partyResponse = await fetch(`/api/party?pageSize=1&search=${encodeURIComponent(bill.partyId)}`);
  ...
  const updateLineItem = (index: number, field: string, value: any) => {
    ...
    if (field === 'quantity' || field === 'weight' || field === 'months' || field === 'rate' || field === 'tax') {
      const qty = field === 'quantity' ? Number(value) : (newLineItems[index].quantity || 0);
      const weight = field === 'weight' ? Number(value) : (newLineItems[index].weight || qty);
      ...
      const taxPercent = field === 'tax' ? Number(value) : newLineItems[index].tax;
    ...
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950 p-6 font-sans">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-neutral-200 dark:border-neutral-800 pb-4">
            <div>
              <h1 className="text-3xl font-bold tracking-tight text-neutral-900 dark:text-neutral-50 uppercase">{activeTab === 'inwards' ? 'Billing Queue' : 'Invoice History'}</h1>
              <p className="text-sm text-neutral-500 dark:text-neutral-400 mt-1">
                {activeTab === 'inwards' ? 'Process pending inwards to generate bills' : 'View and manage generated invoices'}
              </p>
            </div>
        ...
        <div className="bg-white dark:bg-neutral-900 p-5 rounded-xl shadow-sm border border-neutral-200 dark:border-neutral-800 flex flex-col lg:flex-row justify-between items-center gap-4 transition-all hover:shadow-md">
          <div className="flex items-center space-x-3 text-sm text-neutral-600 dark:text-neutral-300 w-full lg:w-auto">
            <label htmlFor="pageSize" className="font-semibold text-neutral-500 uppercase tracking-wider text-[11px]">Show</label>
            ...
            <span className="font-semibold text-neutral-500 uppercase tracking-wider text-[11px]">Entries</span>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
            <div className="relative w-full sm:w-72 group">
              ...
              <input
                ...
                onChange={(e) => activeTab === 'inwards' ? setSearch(e.target.value) : setBillSearch(e.target.value)}
        ...
        <div className="bg-white dark:bg-neutral-900 rounded-2xl shadow-sm border border-neutral-200 dark:border-neutral-800 overflow-hidden">
          <div className="overflow-x-auto">
            {activeTab === 'inwards' ? (
              <table className="min-w-full divide-y divide-neutral-100 dark:divide-neutral-800">
                <thead className="bg-neutral-50/50 dark:bg-neutral-800/50">
                  <tr>
                    ...
                    <th scope="col" className="px-6 py-4 text-left text-[10px] font-black text-neutral-500 dark:text-neutral-400 uppercase tracking-widest">Inward Date</th>
                    <th scope="col" className="px-6 py-4 text-left text-[10px] font-black text-neutral-500 dark:text-neutral-400 uppercase tracking-widest">Total Weight</th>
                    <th scope="col" className="px-6 py-4 text-left text-[10px] font-black text-neutral-500 dark:text-neutral-400 uppercase tracking-widest">Remaining</th>
                    <th scope="col" className="px-6 py-4 text-left text-[10px] font-black text-neutral-500 dark:text-neutral-400 uppercase tracking-widest">Party</th>
                    <th scope="col" className="px-6 py-4 text-left text-[10px] font-black text-neutral-500 dark:text-neutral-400 uppercase tracking-widest">Product</th>
                    <th scope="col" className="px-6 py-4 text-left text-[10px] font-black text-neutral-500 dark:text-neutral-400 uppercase tracking-widest">Condition</th>
                    <th scope="col" className="px-6 py-4 text-center text-[10px] font-black text-neutral-500 dark:text-neutral-400 uppercase tracking-widest">Outward</th>
                    ...
                    <tr>
                      <td colSpan={9} className="px-6 py-20 text-center">
                        <div className="flex flex-col items-center justify-center space-y-3">
                          ...
                          <p className="text-sm font-medium text-neutral-900 dark:text-neutral-100 uppercase tracking-tight">No records found matching your selection</p>
                        </div>
                      </td>
                    </tr>
                  ) : (
                    inwards.map((inward) => (
                      <tr key={inward.id} className="hover:bg-neutral-50/50 dark:hover:bg-neutral-800/30 transition-all group">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-neutral-700 dark:text-neutral-300">
                          {inward.totalWeight.toLocaleString()} <span className="text-[10px] opacity-50 uppercase ml-1">kg</span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          <span className={`inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-tight border shadow-sm
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          <span className="inline-flex items-center text-xs font-semibold text-neutral-600 dark:text-neutral-400">
                            {inward.goodsCondition === 'Good' && <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 mr-2" />}
              ...
              <table className="min-w-full divide-y divide-neutral-100 dark:divide-neutral-800">
                <thead className="bg-neutral-50/50 dark:bg-neutral-800/50">
                  <tr>
                    <th scope="col" className="px-6 py-4 text-left text-[10px] font-black text-neutral-500 dark:text-neutral-400 uppercase tracking-widest">Bill No</th>
                    ...
                    <th scope="col" className="px-6 py-4 text-left text-[10px] font-black text-neutral-500 dark:text-neutral-400 uppercase tracking-widest">Party</th>
                    <th scope="col" className="px-6 py-4 text-right text-[10px] font-black text-neutral-500 dark:text-neutral-400 uppercase tracking-widest">Total Amount (₹)</th>
                    <th scope="col" className="px-6 py-4 text-center text-[10px] font-black text-neutral-500 dark:text-neutral-400 uppercase tracking-widest">Status</th>
                    <th scope="col" className="px-6 py-4 text-right text-[10px] font-black text-neutral-500 dark:text-neutral-400 uppercase tracking-widest">Action</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800 bg-white dark:bg-neutral-900">
                    ...
                    <tr>
                      <td colSpan={6} className="px-6 py-20 text-center">
                        <div className="flex flex-col items-center justify-center space-y-3">
                          ...
                          <p className="text-sm font-medium text-neutral-900 dark:text-neutral-100 uppercase tracking-tight">No bills produced yet</p>
                        </div>
                      </td>
                    </tr>
                  ) : (
                    bills.map((bill) => (
                      <tr key={bill.id} className="hover:bg-neutral-50/50 dark:hover:bg-neutral-800/30 transition-all group">
                          ...
                          <div className="relative flex justify-end items-center group/menu">
                            ...
                            <div className="absolute right-0 top-full mt-2 w-36 bg-white dark:bg-neutral-950 rounded-xl shadow-xl border border-neutral-200 dark:border-neutral-800 opacity-0 invisible group-hover/menu:opacity-100 group-hover/menu:visible peer-focus:opacity-100 peer-focus:visible transition-all z-20 overflow-hidden text-left">
                              <div className="py-1">
                                ...
                                >
                                  <FileText className="w-4 h-4 text-indigo-500" /> View Invoice
                                </button>
                                <button
          ...
          <div className="bg-neutral-50/50 dark:bg-neutral-800/10 px-8 py-5 border-t border-neutral-100 dark:border-neutral-800 flex items-center justify-between">
            <div className="text-[10px] font-black uppercase tracking-widest text-neutral-400">
               {activeTab === 'inwards' ? (
                <>Showing <span className="text-neutral-900 dark:text-neutral-100">{(page - 1) * pageSize + (inwards.length > 0 ? 1 : 0)}</span> — <span className="text-neutral-900 dark:text-neutral-100">{Math.min(page * pageSize, total)}</span> of <span className="text-neutral-900 dark:text-neutral-100">{total}</span></>
               ) : (
                <>Showing <span className="text-neutral-900 dark:text-neutral-100">{(billPage - 1) * billPageSize + (bills.length > 0 ? 1 : 0)}</span> — <span className="text-neutral-900 dark:text-neutral-100">{Math.min(billPage * billPageSize, billTotal)}</span> of <span className="text-neutral-900 dark:text-neutral-100">{billTotal}</span></>
               )}
            </div>
            <div className="flex space-x-3">
              <button
                onClick={() => activeTab === 'inwards' ? setPage(p => Math.max(1, p - 1)) : setBillPage(p => Math.max(1, p - 1))}
              ...
              <button
                onClick={() => activeTab === 'inwards' ? setPage(p => Math.min(totalPages, p + 1)) : setBillPage(p => Math.min(Math.ceil(billTotal / billPageSize), p + 1))}
                disabled={activeTab === 'inwards' ? (page === totalPages || totalPages === 0) : (billPage === Math.ceil(billTotal / billPageSize) || billTotal === 0)}
                className="p-2.5 rounded-xl border border-neutral-200 dark:border-neutral-700 text-neutral-500 hover:bg-white dark:hover:bg-neutral-800 disabled:opacity-30 disabled:cursor-not-allowed transition-all shadow-sm active:scale-90"
              >
                <ChevronRight className="w-4 h-4" />
              </button>
            </div>
        </div> {/* Pagination (564) Close */}
      </div> {/* Table Area (370) Close */}
    </div> {/* max-w-7xl (276) Close */}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-neutral-900 rounded-2xl shadow-xl w-full max-w-2xl border border-neutral-200 dark:border-neutral-800 overflow-hidden text-left flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleSaveInward} className="flex flex-col flex-1 overflow-y-auto p-6 text-sm">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="block font-medium text-neutral-700 dark:text-neutral-300">Inward Date <span className="text-red-500">*</span></label>
                  <input
                    type="date"
                    required
                ...
                <div className="space-y-2">
                  <label className="block font-medium text-neutral-700 dark:text-neutral-300">Party <span className="text-red-500">*</span></label>
                  <select
                    required
                ...
                <div className="space-y-2">
                  <label className="block font-medium text-neutral-700 dark:text-neutral-300">Product <span className="text-red-500">*</span></label>
                  <select
                    required
                ...
                <div className="space-y-2">
                  <label className="block font-medium text-neutral-700 dark:text-neutral-300">Total Weight (kg) <span className="text-red-500">*</span></label>
                  <input
                    type="number"
                    required
                    min={0}
                    ...
                    onChange={(e) => setFormData({ 
                      ...
                      remainingWeight: editingInward ? formData.remainingWeight : Number(e.target.value) 
                    })}
                    className="w-full px-3 py-2 bg-white dark:bg-neutral-950 border border-neutral-300 dark:border-neutral-700 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                  />
                </div>
                ...
                <div className="space-y-2">
                  <label className="block font-medium text-neutral-700 dark:text-neutral-300">Remaining Weight (kg)</label>
                  <input
                    type="number"
                    min={0}
                    value={formData.remainingWeight !== undefined ? formData.remainingWeight : ''}
                    onChange={(e) => setFormData({ ...formData, remainingWeight: Number(e.target.value) })}
                    className="w-full px-3 py-2 bg-white dark:bg-neutral-950 border border-neutral-300 dark:border-neutral-700 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                  />
                </div>
      ...
      {isBillModalOpen && billPreview && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-neutral-900/60 backdrop-blur-md" onClick={() => setIsBillModalOpen(false)}></div>
          <div className="relative bg-white dark:bg-neutral-900 rounded-4xl shadow-2xl w-full max-w-5xl border border-neutral-200 dark:border-neutral-800 overflow-hidden flex flex-col max-h-[95vh] animate-in fade-in zoom-in duration-300">
            ...
            <div className="px-8 py-6 border-b border-neutral-100 dark:border-neutral-800 flex justify-between items-center bg-white dark:bg-neutral-900 relative z-10">
              <div>
                ...
                <p className="text-xs font-bold text-neutral-400 uppercase tracking-widest mt-1">Invoice Generation for {billPreview.partyId}</p>
              </div>
              <button onClick={() => setIsBillModalOpen(false)} className="p-2 rounded-full hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors">
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-neutral-400">Bill Number</label>
                  <input
                    type="text"
                    value={billPreview.billNumber}
                    onChange={(e) => setBillPreview({ ...billPreview, billNumber: e.target.value })}
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-neutral-400">Invoice Date</label>
                  <input
                    type="date"
                    value={billPreview.date}
                    onChange={(e) => setBillPreview({ ...billPreview, date: e.target.value })}
                    className="w-full px-4 py-3 bg-neutral-50 dark:bg-neutral-950 border border-neutral-100 dark:border-neutral-800 rounded-2xl font-bold text-neutral-900 dark:text-neutral-100 focus:ring-2 focus:ring-indigo-500 outline-none"
                  />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-neutral-400">Party ID</label>
                  <p className="px-4 py-3 bg-neutral-100 dark:bg-neutral-800 rounded-2xl font-black text-indigo-600 dark:text-indigo-400 text-sm">{billPreview.partyId}</p>
                </div>
              </div>
                ...
                <table className="w-full text-left text-sm border-collapse">
                  <thead>
                    <tr className="border-b border-neutral-100 dark:border-neutral-800">
                      <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-neutral-400">Description</th>
                      <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-neutral-400 w-32">Weight/Qty</th>
                      <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-neutral-400 w-24">Months</th>
                      <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-neutral-400 w-32">Rate (₹)</th>
                      <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-neutral-400 w-24">Tax (%)</th>
                      <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-neutral-400 text-right w-40">Total (₹)</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800">
                    {billPreview.lineItems.map((item: any, idx: number) => (
                      <tr key={idx} className="group hover:bg-white dark:hover:bg-neutral-900 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <input
                            type="number"
                            value={item.months || 1}
                            onChange={(e) => updateLineItem(idx, 'months', e.target.value)}
                            className="w-full bg-neutral-100 dark:bg-neutral-800/50 border-0 rounded-lg px-2 py-1.5 font-bold focus:ring-1 focus:ring-indigo-500 outline-none"
                          />
                        </td>
                        <td className="px-6 py-4">
              ...
              <div className="flex flex-col md:flex-row justify-between gap-12 pt-4">
                <div className="flex-1 space-y-4">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-neutral-400">Remarks / Terms</label>
                    <textarea
                        rows={4}
                        placeholder="Add payment terms or notes..."
                        value={billPreview.remarks}
                        onChange={(e) => setBillPreview({ ...billPreview, remarks: e.target.value })}
                ...
                <div className="w-full md:w-80 space-y-3 bg-neutral-900 dark:bg-black rounded-4xl p-8 text-white shadow-xl transform hover:scale-[1.02] transition-transform">
                  <div className="flex justify-between items-center opacity-60">
                    ...
                    <span className="font-bold font-mono">₹{billPreview.subTotal?.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                  </div>
                  <div className="flex justify-between items-center opacity-60">
                    <span className="text-[10px] font-black uppercase tracking-widest">Tax Amount</span>
                    <span className="font-bold font-mono">₹{billPreview.taxTotal?.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                  </div>
                  <div className="h-px bg-white/10 my-4" />
                  <div className="flex justify-between items-end">
                    <div className="space-y-1">
                        <span className="text-[10px] font-black uppercase tracking-widest text-indigo-400">Grand Total</span>
                        <p className="text-4xl font-black tracking-tighter font-mono">₹{billPreview.grandTotal?.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {isEditBillModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-neutral-900/60 backdrop-blur-md" onClick={() => setIsEditBillModalOpen(false)}></div>
          <div className="relative bg-white dark:bg-neutral-900 rounded-4xl shadow-2xl w-full max-w-4xl border border-neutral-200 dark:border-neutral-800 overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in duration-300">
            <div className="px-8 py-6 border-b border-neutral-100 dark:border-neutral-800 flex justify-between items-center bg-white dark:bg-neutral-900">
              <div>
                <h3 className="text-2xl font-black text-neutral-900 dark:text-neutral-50 tracking-tighter">Edit Bill: {billFormData.billNumber}</h3>
                <p className="text-[10px] font-black uppercase tracking-widest text-neutral-400 mt-1">Update invoice details and status</p>
              </div>
              <button onClick={() => setIsEditBillModalOpen(false)} className="p-2 rounded-full hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors">
                <X className="w-6 h-6 text-neutral-400" />
              </button>
            </div>
            
            <form onSubmit={handleUpdateBill} className="flex-1 overflow-y-auto p-8 space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-neutral-400">Outward Date</label>
                  <div className="relative">
                    <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-400" />
                    <input
                      type="date"
                      value={billFormData.outwardDate || ''}
                      onChange={(e) => setBillFormData({ ...billFormData, outwardDate: e.target.value })}
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-neutral-400">Storage Months</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) => setBillFormData({ ...billFormData, storageMonths: Number(e.target.value) })}
                      className="w-full pl-10 pr-4 py-3 bg-neutral-50 dark:bg-neutral-950 border border-neutral-100 dark:border-neutral-800 rounded-2xl font-bold focus:ring-2 focus:ring-indigo-500 outline-none"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-neutral-400">GST (%)</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) => setBillFormData({ ...billFormData, gst: Number(e.target.value) })}
                      className="w-full pl-10 pr-4 py-3 bg-neutral-50 dark:bg-neutral-950 border border-neutral-100 dark:border-neutral-800 rounded-2xl font-bold focus:ring-2 focus:ring-indigo-500 outline-none"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <select
                      ...
                      onChange={(e) => setBillFormData({ ...billFormData, paymentStatus: e.target.value as any })}
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <select
                      ...
                      onChange={(e) => setBillFormData({ ...billFormData, paymentMode: e.target.value })}
                      className="w-full pl-10 pr-4 py-3 bg-neutral-50 dark:bg-neutral-950 border border-neutral-100 dark:border-neutral-800 rounded-2xl font-bold focus:ring-2 focus:ring-indigo-500 outline-none appearance-none"
                    >
              ...
              <div className="space-y-2 pt-4">
                <label className="text-[10px] font-black uppercase tracking-widest text-neutral-400">Remarks</label>
                <textarea
                  rows={3}
                ...
                <div className="border border-black flex flex-col h-full relative">
                  ...
                  <div className="flex border-b border-black">
                    <div className="w-1/3 p-4 flex flex-col items-center justify-center border-r border-black relative">
                      ...
                      <p className="text-xl font-bold text-blue-900 mt-1 whitespace-nowrap uppercase">COLD STORAGE</p>
                      <div className="absolute right-4 top-4 text-blue-900 opacity-20 invisible">
                         {/* SVG icon from inwards page could go here */}
                      </div>
                    </div>
                  ...
                  <div className="flex-1 border-b border-black font-semibold text-[10px] flex flex-col divide-y divide-black/20 min-h-[300px]">
                    {invoicePreviewData.bill.lineItems.map((item: any, idx: number) => (
                      <div key={idx} className="flex text-center divide-x divide-black/40 min-h-[28px] py-1.5 items-center">
                        ...
                        <div className="flex-[1.5] px-2">{item.weight || item.quantity}</div>
                        <div className="flex-[1.5] px-2">{item.remaining || 0}</div>
                        <div className="flex-[1.5] px-2">{Number(item.rate).toFixed(2)}</div>
                        <div className="flex-[1.2] px-2">{item.months || 1}</div>
                        <div className="flex-[2] px-2 text-right">{Number(item.total).toFixed(2)}</div>
                      </div>
                    ))}
                  ...
                  <div className="flex border-b border-black h-36">
                    <div className="w-1/2 p-4 font-semibold text-xs border-r border-black flex flex-col gap-1.5 font-mono leading-tight bg-slate-50/30">
                      <div className="flex"><span className="w-24 inline-block font-bold text-slate-500">Remarks :</span> <span className="text-black uppercase">{invoicePreviewData.bill.remarks || 'COLD RENT'}</span></div>
                      <div className="flex"><span className="w-24 inline-block font-bold text-slate-500">Bank Detail:</span> <span className="text-black uppercase">Canara Bank</span></div>
                      <div className="flex"><span className="w-24 inline-block font-bold text-slate-500">Branch:</span> <span className="text-black uppercase">Hazira</span></div>
                      <div className="flex"><span className="w-24 inline-block font-bold text-slate-500">A/C Number:</span> <span className="text-black font-black">120029409483</span></div>
                      <div className="flex"><span className="w-24 inline-block font-bold text-slate-500">IFSC Code:</span> <span className="text-black font-black uppercase">CNRB0003428</span></div>
                    </div>
                    <div className="w-1/2 flex flex-col font-bold text-xs p-4 font-mono justify-between bg-white">
                      <div className="space-y-2">
                          <div className="flex justify-between items-center"><span className="text-slate-500 font-bold uppercase tracking-tighter">Sub Total:</span> <span className="text-base text-black">₹{invoicePreviewData.bill.subTotal.toFixed(2)}</span></div>
                          {invoicePreviewData.bill.taxTotal > 0 && (
                            <>
                                <div className="flex justify-between items-center"><span className="text-slate-500 font-bold uppercase tracking-tighter">SGST Total:</span> <span className="text-black">₹{(invoicePreviewData.bill.taxTotal / 2).toFixed(2)}</span></div>
                                <div className="flex justify-between items-center"><span className="text-slate-500 font-bold uppercase tracking-tighter">CGST Total:</span> <span className="text-black">₹{(invoicePreviewData.bill.taxTotal / 2).toFixed(2)}</span></div>
                            </>
                          )}
                      </div>
                      <div className="flex justify-between items-center border-t border-black pt-2 mt-2 text-lg font-black bg-slate-900 text-white p-2 rounded-lg">
                        <span className="uppercase tracking-tighter text-xs text-indigo-300">Net Amount:</span>
                  ...
                  <div className="border-b border-black p-3 font-bold text-[11px] uppercase font-mono bg-slate-100 flex items-center gap-3">
                    <span className="text-slate-500 font-black italic">Amount in words:</span>
                    <span className="text-black">{numberToWords(invoicePreviewData.bill.grandTotal)}</span>
                  </div>
                  ...
                  <div className="flex h-32">
                    <div className="w-2/3 p-3 text-[9px] leading-relaxed border-r border-black flex flex-col justify-start text-slate-700 bg-white">
                      <p className="font-black mb-1 text-black uppercase tracking-widest border-b border-black/10 inline-block">Terms & Conditions:</p>
                      <p>1. Any complaint about this tax invoice must be lodged within two working days.</p>
                      <p>2. Payment to be made in favour of JCRM Cold Storage LLP.</p>
                      <p>3. Interest @24% p.a. will be charged if not paid within 7 days.</p>
                    ...
                    <div className="w-1/3 flex flex-col justify-between items-center p-4 text-[11px] bg-slate-50/50">
                      ...
                      <div className="w-full border-t-2 border-slate-900 text-center font-black pt-2 uppercase tracking-tighter">
                        ...
                        <div className="text-[9px] font-bold text-slate-500 mt-1 capitalize">For JCRM Cold Storage LLP</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="absolute -bottom-6 w-full text-center text-[9px] font-bold text-slate-400 print:bottom-[-25px] uppercase tracking-widest">
                    SUBJECT TO SURAT JURISDICTION — THIS IS A COMPUTER GENERATED DOCUMENT
                  </div>
                </div>
              </div>
            </div>
